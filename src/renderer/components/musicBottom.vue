...
export default{
  ...
  methods: {
    ...
    _createtooltip (mes) {
      ...
      let wrap = document.querySelector('#mlist')
      span.style = 'position:absolute; bottom: 25px; border-radius: 5px/10px; left:-15px ; width:70px; height:20px; background-color: darkslategrey; line-height: 19px; text-align:center'
      span.innerText = mes
      let i = document.createElement('i')
      i.style = `position:absolute; width: 0; height: 0; border-left: 7px solid transparent; border-right: 7px solid transparent; border-top: 7px solid rgba(0, 0, 0, .5); bottom: -7px; left: 30px`
  ...
  created () {
    ...
    this.$root.eventHub.$on('addTosonglist', function (list) {
      let mes
      if (!list.errno) {
        vm.musiclist = vm.musiclist.concat(list.data) //  链接新加入的歌曲
        mes = '添加成功'
      } else {
        mes = '已经添加'
      }
      vm._createtooltip(mes)
    })
  },
  mounted () {
...
<style>
  ...
  #playlist {
    ...
    height: 50%;
    right: 10px;
    box-shadow: -4px -3px 11px rgba(183, 43, 43, 0.5);
    text-align: center;
    border-radius: 5px;
